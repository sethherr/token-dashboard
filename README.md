...
| `TOKEN_DASHBOARD_GIT_BIN` | auto-detected | Path to `git`, used by the optional workspace→PR association. |
| `TOKEN_DASHBOARD_GH_BIN` | auto-detected | Path to the `gh` CLI, used by the optional workspace→PR association. |
...
- **RTK** — optional [rtk](https://github.com/rtk-ai/rtk) savings view when the CLI is installed. The binary is located via `PATH` first, then common install dirs (`~/.local/bin`, `~/.cargo/bin`, `/opt/homebrew/bin`, `/usr/local/bin`); set `TOKEN_DASHBOARD_RTK_BIN=/path/to/rtk` to point at it explicitly. Gracefully degrades when RTK is absent (shows install instructions).
- **Workspace → GitHub PR association** *(Settings, off by default)* — when on, every workspace is relabelled `{repo}: #{number} - {title}` in Projects, Sessions, and Workspaces instead of showing the directory name, with the `#{number}` linking to the PR on GitHub. Built for git-worktree tooling (Conductor and friends) that names directories `dubai-v3` or `nukualofa`, where the folder tells you nothing about the work. **Deleted worktrees resolve too** — the branch survives in the transcripts (`gitBranch`) and GitHub keeps merged PRs, so history that no longer exists on disk still gets its PR title. The main checkout shows `{repo}: main worktree`; a worktree with no PR falls back to `{repo}: {branch}`. Every workspace name — PR-labelled or not — carries a `?` icon that reveals the directory it stands for, on hover and on click. Normal refreshes top up new workspaces automatically; **Refresh PR links** in Settings re-resolves everything. Uses your local `git` and `gh` CLIs — see [limitations](docs/KNOWN_LIMITATIONS.md#workspace-pr-association-what-can-and-cant-be-resolved).
...
- **Settings** — pick your plan (API / Pro / Max / Max-20x / Team). This does **not** change the dollar figures shown elsewhere — they're always the API pay-per-token value of your usage — it just shows your flat monthly fee next to them for comparison, so you can see whether your usage is worth more than you pay. Also lets you switch the `.claude` folder at runtime, and turn on **workspace→GitHub PR association** (below).
