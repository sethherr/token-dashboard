...
- **Workspace → GitHub PR association** *(Settings, off by default)* — when on, every workspace is relabelled `{repo}: PR #{number} - {title}` in Projects, Sessions, and Workspaces instead of showing the directory name. Built for git-worktree tooling (Conductor and friends) that names directories `dubai-v3` or `nukualofa`, where the folder tells you nothing about the work. The main checkout shows `{repo}: main worktree`; a worktree with no PR falls back to `{repo}: {branch}`. Hover or click any workspace name to see its path. Uses your local `git` and `gh` CLIs — see [limitations](docs/KNOWN_LIMITATIONS.md#workspace-pr-association-only-covers-worktrees-that-still-exist).
