...
- **Workspace → GitHub PR association** *(Settings, off by default)* — when on, every workspace is relabelled `{repo}: PR #{number} - {title}` in Projects, Sessions, and Workspaces instead of showing the directory name. Built for git-worktree tooling (Conductor and friends) that names directories `dubai-v3` or `nukualofa`, where the folder tells you nothing about the work. **Deleted worktrees resolve too** — the branch survives in the transcripts (`gitBranch`) and GitHub keeps merged PRs, so history that no longer exists on disk still gets its PR title. The main checkout shows `{repo}: main worktree`; a worktree with no PR falls back to `{repo}: {branch}`. Hover or click any workspace name to see its path. Normal refreshes top up new workspaces automatically; **Refresh PR links** in Settings re-resolves everything. Uses your local `git` and `gh` CLIs — see [limitations](docs/KNOWN_LIMITATIONS.md#workspace-pr-association-what-can-and-cant-be-resolved).
