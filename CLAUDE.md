...
- **Session-window queries need the session-first index.** The per-prompt and per-skill aggregates all ask "rows of this type in this session between two timestamps". `idx_messages_session_type_ts` is what makes them cheap — without it the planner drives off `(type, timestamp)` and walks every later message in every other session (`/api/skills` went 91s → 2s on a 206k-message DB). Keep new correlated aggregates in that column order, and check `EXPLAIN QUERY PLAN` picks it up.
- **Long tables collapse themselves.** `web/tables.js` wraps any table with more than 10 body rows in a scrolling frame with a floating expand/collapse toggle — routes render plain `<table>` markup and get it for free, including tables added after the first render. Put totals in a `<tfoot>` so they stay pinned below the frame; add `data-no-collapse` to opt a table out.
